...
export async function showEncounter(){
  ...
  <section class="container narrow">
    ...
    <div class="card p16 mt16" id="panel">
      ...
      <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:14px">
        <button class="btn" id="btnStart">조우 시작</button>
      </div>
    </div>
  ...
  async function startMatch(){
    ...
    try{
      ...
      const { data } = await call({ charId: me.id, mode: 'encounter' });
      ...
      `;
      statusEl.textContent = '상대가 정해졌어! 이제 조우 진행만 하면 돼.';
      ...
      // (옵션) 조우 로그 미리보기 — 추후 ai.js에 genEncounterEpisode 추가시 교체
      // await previewEncounterAI(me, opp);
