...
async function onSearch(){
  ...
  try{
    ...
    list.innerHTML = arr.map(u=>{
      ...
      // [수정] 사용자 설정 프로필(avatar_b64)을 우선 사용하도록 변경
      const avatarSrc = u.avatar_b64 || u.avatarURL || '';
      return `
        <div class="row between item">
          <div class="person btnOpen" data-uid="${u.uid}" data-nick="${escapeHtml(u.nickname||'')}" data-ava="${avatarSrc}" title="프로필/캐릭터 보기">
            <div class="avatar avatar-sm"><img src="${avatarSrc}" alt=""/></div>
            <div class="nick">${escapeHtml(u.nickname||'(이름없음)')}</div>
          </div>
          <div class="row gap8">
            <button class="btnAdd" data-uid="${u.uid}" ${disabled?'disabled':''}>${btnLabel}</button>
            <button class="btnOpen" data-uid="${u.uid}" data-nick="${escapeHtml(u.nickname||'')}" data-ava="${avatarSrc}">열기</button>
          </div>
        </div>`;
...
async function refreshIncoming(){
  ...
  try{
    ...
    const infos = await Promise.all(arr.map(async r=>{
      ...
      // [수정] 사용자 설정 프로필(avatar_b64)을 우선 사용하도록 변경
      const avatarSrc = u?.avatar_b64 || u?.avatarURL || '';
      return { ...r, fromNick:u?.nickname||'(이름없음)', fromAva: avatarSrc };
    }));
...
async function refreshOutgoing(){
  ...
  try{
    ...
    const infos = await Promise.all(arr.map(async r=>{
      ...
      // [수정] 사용자 설정 프로필(avatar_b64)을 우선 사용하도록 변경
      const avatarSrc = u?.avatar_b64 || u?.avatarURL || '';
      return { ...r, toNick:u?.nickname||'(이름없음)', toAva: avatarSrc };
    }));
        ...
        <div class="fr-info">
          ...
          <div class="fr-actions">
            <button data-uid="${u.uid}" data-nick="${escapeHtml(u.nickname||'(이름없음)')}"
                    data-ava="${avatarSrc}" class="btnOpen">열기</button>
            <button data-uid="${u.uid}" class="btnUnf">삭제</button>
          </div>
        </div>
        <div class="fr-photo"><img src="${avatarSrc}" alt=""/></div>
      </div>`
