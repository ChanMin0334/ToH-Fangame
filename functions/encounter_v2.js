...
function toSec(v) {
  if (typeof v === 'number') return Math.floor(Number(v) || 0);
...
module.exports = (admin, { HttpsError, logger }) => {
  ...
  const startEncounter = onCall({ secrets: [GEMINI_API_KEY], region: 'us-central1' }, async (req) => {
    ...
    // 1) 공용 쿨타임 검사 (숫자/Timestamp 모두 대응)
    const userSnap = await db.collection('users').doc(uid).get();
    const userData = userSnap.data() || {};
    const nowSec = Math.floor(Date.now() / 1000);
    const cooldownUntil = toSec(userData.cooldown_all_until);
    ...
    try {
      ...
      // 상대 캐릭터: 트랜잭션/관계노트/모의조우에서 공통 사용 (스코프 버그 방지)
      const opponentCharSnap = await db.collection('chars').doc(opponentCharId).get();
...
${relationNote}
      ...
      await db.runTransaction(async (tx) => {
        ...
        // 트랜잭션 내 상대 데이터는 변수명 충돌 방지용(oppData)
        const oppShot = await tx.get(charBRef);
        if (!oppShot.exists) throw new HttpsError('not-found', '상대 캐릭터를 찾을 수 없습니다.');
        const oppData = oppShot.data();

        const userARef = db.collection('users').doc(myChar.owner_uid);
        const userBRef = db.collection('users').doc(oppData.owner_uid);
        ...
        tx.set(logRef, {
          ...
          b_char: `chars/${opponentCharId}`,
          a_snapshot: { name: myChar.name, thumb_url: myChar.thumb_url || null },
          b_snapshot: { name: oppData.name, thumb_url: oppData.thumb_url || null },
          title: result.title,
          content: result.content,
          exp_a: expA,
          exp_b: expB,
      ...
      // 7) 관계 노트 생성 (트랜잭션 밖, 공통 스코프 변수 사용)
      try {
...
${result.content}
        `;
        const relResult = await callGemini({
          apiKey: GEMINI_API_KEY.value(),
          systemText: relSystemPrompt,
          userText: relUserPrompt,
          logger,
          modelName: primary
        });
        ...
        if (relResult && relResult.note) {
          ...
          await baseRef.set(
            { pair: sortedPair, updatedAt: FieldValue.serverTimestamp(), lastEncounterLogId: logRef.id },
            { merge: true }
          );
          await noteRef.set(
            { note: relResult.note, updatedAt: FieldValue.serverTimestamp(), updatedBy: uid },
            { merge: true }
          );
